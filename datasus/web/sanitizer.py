...
for arquivo in arquivos_tab:
    # Ler o arquivo .tab
    df = pd.read_csv(os.path.join(diretorio, arquivo), sep='\t', encoding='utf-8', skiprows=4)

    # Realizar as alterações necessárias no DataFrame df

    # Salvar o DataFrame com o mesmo nome do arquivo original
    df.to_csv(os.path.join(diretorio_saida, arquivo), sep=';', index=False, encoding='iso-8859-15')
    conta_arquivos += 1
